...
FullScreen.prototype.updateActiveIndex = function(newActiveIndex) {
  var that = this
  var translate = that.rtlTranslate ? that.translate : -that.translate
  var slidesGrid = that.slidesGrid
  ...
  that.emit('activeIndexChange')
  that.emit('snapIndexChange')

  if (that.initialized) {
    that.emit('slideChange')
  }
}
...
FullScreen.prototype.prependSlide = function(slides) {
  ...
  that.slideTo(newActiveIndex, 0, false)
}
...
function onTouchEnd(event) {
  var that = this
  var data = that.touchEventsData
  console.log('data.isMoved -> ', data.isMoved)
  // debugger
  var params = that.params
  var touches = that.touches
  var rtl = that.rtlTranslate
  ...
  // Tap, doubleTap, Click
  // that.emit('tap click', e)
  // if (timeDiff < 300 && touchEndTime - data.lastClickTime < 300) {
  //   that.emit('doubleTap doubleClick', e)
  // }
